...
from dgl.nn.pytorch.conv import GATConv, EdgeGATConv
...
class ResidualEdgeGATConv(torch.nn.Module):
    def __init__(
        ...
        if self.edge_feats is None:
            ...
            self.conv = EdgeGATConv(in_feats,edge_feats, out_feats, num_heads,
                                    allow_zero_in_degree=True, residual=False, bias=False)
    ...
    def forward(self, graph, feat, edge_feat=None, get_attention=False):
        ...
        # pad to reach initial out size
        #x = F.pad(x,(0,self.out_feats-x.shape[1]),'constant',0)
